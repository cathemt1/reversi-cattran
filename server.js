    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        io.in(room).fetchSockets().then((sockets) => {
            ...
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.includes(socket)) {
                response = {};
                console.log('entered the fail statement for ifs');
                response.result = 'fail';
    ...
    socket.on('invite', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (room === null) || (requested_user === "")) {
            response = {
                result: 'fail',
                message: 'client did not a valid user to invite to play'
            }
            socket.emit('invite_response', response);
            serverLog('invite command failed', JSON.stringify(response));
            return;
        ...
        if ((typeof room == 'undefined') || (username === null) || (room === "")) {
            response = {
                result: 'fail',
    ...
    socket.on('uninvite', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (room === null) || (requested_user === "")) {
            response = {
                result: 'fail',
                message: 'client did not a valid user to uninvite'
            }
            socket.emit('uninvited', response);
            serverLog('uninvite command failed', JSON.stringify(response));
            return;
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (room === null) || (requested_user === "")) {
            response = {
                result: 'fail',
                message: 'client did not a valid user to engage in play'
            }
            socket.emit('game_start_response', response);
            serverLog('game_start command failed', JSON.stringify(response));
            return;
...
function check_line_match(color,dr,dc,r,c,board){
    ...
    /*check to make sure we arent going to walk off the board*/
    if((r+dr<0)||(r+dr>7)){
        return false;
    }
    if((c+dc<0)||(c+dc>7)){
